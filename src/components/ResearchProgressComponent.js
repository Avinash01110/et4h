...
import { createResearchProgress, updateResearchProgress, deleteSingleImage, getResearchProgress } from '../services/operations/researchProgressAPI';
...
const ResearchProgressComponent = ({ postId, token }) => {
    ...
    const handleDeleteImage = async (url) => {
        const result = await deleteSingleImage({ id: researchProgress._id, url }, token);
        if (result) {
    ...
    return (
        <div className="research-progress-component">
            <h2>Research Progress</h2>

            {/* Show Add or Update Button based on the presence of research progress */}
            {researchProgress ? (
                <button onClick={() => setIsModalOpen(true)}>Update Research Progress</button>
            ) : (
                <button onClick={() => setIsModalOpen(true)}>Add Research Progress</button>
            )}
            ...
            >
                <h2>{researchProgress ? "Update" : "Add"} Research Progress</h2>
                <textarea
                    value={description}
                ...
                <select
                    multiple
                    value={contributors}
                    onChange={(e) => setContributors([...e.target.selectedOptions].map(option => option.value))}
                >
                    {loading ? (
                ...
                <button onClick={handleSaveResearchProgress}>
            ...
            {researchProgress ? (
                <div>
                    ...
                    {researchProgress.contributors && researchProgress.contributors.length > 0 ? (
                ...
                <p>No research progress available.</p>
            )}
        </div>
    );
};
